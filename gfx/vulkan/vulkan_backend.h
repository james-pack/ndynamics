...
/**
 * VulkanBackend implements the Backend interface using Vulkan 1.2.
 * It manages Vulkan instance, device, queues, and command buffers to execute
 * compute and render pipelines safely and efficiently. All resources are RAII-managed.
 */
class VulkanBackend final : public Backend {
 ...
 public:
  ...
  std::unique_ptr<ShaderModule> create_shader(ShaderUsage usage, const void* bytecode,
  ...
  void draw(CommandBuffer& cmd, RenderPipeline& pipeline, Buffer& vertex_buffer,
            Buffer* index_buffer, std::uint32_t vertex_or_index_count) override;
