...
inline PGA3D operator&(const PGA3D &a, const PGA3D &b) {
  ...
  // Note: Changed code for the coefficients of the vectors (res[1], res[2], res[3], and res[4])
  // below. Previous code was not consistent with the outer product. The regressive product should
  // be the dual of the outer product of the duals of the operands. The previous code was incorrect
  // and did not maintain that invariant.
  // Previous code:
  ...
  res[4] = 1 * (a[4] * b[15] - a[7] * b[14] + a[9] * b[13] - a[10] * b[12] - a[12] * b[10] +
                a[13] * b[9] - a[14] * b[7] + a[15] * b[4]);

  res[3] = 1 * (a[3] * b[15] - a[6] * b[14] - a[8] * b[13] + a[10] * b[11] + a[11] * b[10] -
                a[13] * b[8] - a[14] * b[6] + a[15] * b[3]);

  res[2] = 1 * (a[2] * b[15] - a[5] * b[14] + a[8] * b[12] - a[9] * b[11] - a[11] * b[9] +
                a[12] * b[8] - a[14] * b[5] + a[15] * b[2]);

  res[1] = 1 * (a[1] * b[15] + a[5] * b[13] + a[6] * b[12] + a[7] * b[11] + a[11] * b[7] +
                a[12] * b[6] + a[13] * b[5] + a[15] * b[1]);
