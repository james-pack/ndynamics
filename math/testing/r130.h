...
class R130 {
 public:
  ...
  R130() { std::fill(mvec, mvec + sizeof(mvec) / 4, 0.0f); }
  R130(float f, int idx = 0) {
    std::fill(mvec, mvec + sizeof(mvec) / 4, 0.0f);
    mvec[idx] = f;
  }

  R130(const R130 &rhs) = default;
  R130(R130 &&rhs) = default;

  bool operator==(const R130 &rhs) const {
...
template <>
class BivectorNetTypes<1, 3, 0> final {
 public:
  using type = R130;
};
...
//***********************
inline R130 operator~(const R130 &a) {
  R130 res;
  res[0] = a[0];
  res[1] = a[1];
  res[2] = a[2];
...
//***********************
// R130.Conjugate : res = a.Conjugate()
// Clifford Conjugation
//***********************
inline R130 R130::Conjugate() {
  R130 res;
  res[0] = this->mvec[0];
  res[1] = -this->mvec[1];
  res[2] = -this->mvec[2];
...
//***********************
// R130.Involute : res = a.Involute()
// Main involution
//***********************
inline R130 R130::Involute() {
  R130 res;
  res[0] = this->mvec[0];
  res[1] = -this->mvec[1];
  res[2] = -this->mvec[2];
...
//***********************
inline R130 operator*(const R130 &a, const R130 &b) {
  R130 res;
...
//***********************
inline R130 operator*(const float &a, const R130 &b) {
  R130 res;
  res[0] = a * b[0];
  res[1] = a * b[1];
  res[2] = a * b[2];
...
//***********************
inline R130 operator*(const R130 &a, const float &b) {
  R130 res;
  res[0] = a[0] * b;
  res[1] = a[1] * b;
  res[2] = a[2] * b;
...
inline float R130::norm() { return sqrt(std::abs(((*this) * Conjugate()).mvec[0])); }
inline float R130::inorm() { return (!(*this)).norm(); }
inline R130 R130::normalized() { return (*this) * (1 / norm()); }
