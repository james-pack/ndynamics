...
class Multivector final {
 ...
 public:
  ...
  /**
   * The dual of a multivector is often given as the left contraction of that multivector with the
   * reversed pseudoscalar. But this approach fails in algebras with degenerate bases. In those
   * cases, we need to revert back to the fundamental definition -- the dual of a unit multivector
   * is its multiplicative inverse. More fully, for a non-unit multivector, the product of the
   * multivector and its dual is the multiplicative identity times the norm squared.
   */
  constexpr Multivector dual() const {
    Multivector result{};
    for (size_t i = 0; i < NUM_BASIS_BLADES; ++i) {
      const size_t complement_basis((NUM_BASIS_BLADES - 1) & (~i));
      const auto& cayley_entry{cayley_table_.entry(i, complement_basis)};
      result.coefficients_[complement_basis] = cayley_entry.structure_constant * coefficients_[i];
    }
    return result;
  }
   ...
   *
   * The test has two stages. First, the multivector must be purely even-grade or purely
   * odd-grade; a mixed-grade element cannot be a versor and is rejected cheaply before the more
   * expensive check. Second, the geometric product X * ~X must be a pure scalar — higher-grade
   * components in this product indicate that the versor formula would silently discard
   * algebraically significant terms. The scalar must also be nonzero, as null elements (ideal
   * points and lines in PGA, for example) are not invertible and therefore not versors in the
   * sense required by inverse().
   *
  ...
  // Equality. Note that these operators implement exact equality. Use near_equal() below for a
  // more reasonable equality check.
   ...
   * the Cayley tables for each algebra. It can also make for consistent indexing across algebras,
   * except where particular conventions in a specific algebra force deviations from the pattern.
   *
