...
TEST(GeometricProductCayleyEntryCalculatorTest, CorrectCommutativeOrderForSplitComplexNumbers) {
  ...
  EXPECT_EQ(1, calculator.compute_commutative_order(0, 0));
  EXPECT_EQ(1, calculator.compute_commutative_order(0, 1));
  EXPECT_EQ(1, calculator.compute_commutative_order(1, 0));
  EXPECT_EQ(1, calculator.compute_commutative_order(1, 1));
}
...
TEST(GeometricProductCayleyEntryCalculatorTest, CorrectCommutativeOrderForDualNumbers) {
  ...
  EXPECT_EQ(1, calculator.compute_commutative_order(0, 1));
  EXPECT_EQ(1, calculator.compute_commutative_order(1, 0));
  EXPECT_EQ(0, calculator.compute_commutative_order(1, 1));
}
...
TEST(GeometricProductCayleyEntryCalculatorTest, CorrectCommutativeOrderForVgaNumbers) {
  ...
  EXPECT_EQ(1, calculator.compute_commutative_order(4, 0));
  EXPECT_EQ(1, calculator.compute_commutative_order(4, 4));

  // 1 commute.
  EXPECT_EQ(-1, calculator.compute_commutative_order(4, 1));

  // 1 commute.
  EXPECT_EQ(-1, calculator.compute_commutative_order(5, 2));

  // 2 commutes.
  EXPECT_EQ(1, calculator.compute_commutative_order(6, 1));

  // 3 commutes.
  EXPECT_EQ(-1, calculator.compute_commutative_order(7, 7));
}
...
TEST(GeometricProductCayleyEntryCalculatorTest, CorrectCommutativeOrderForStaNumbers) {
  ...
  EXPECT_EQ(1, calculator.compute_commutative_order(2, 0));
  EXPECT_EQ(1, calculator.compute_commutative_order(2, 4));
  EXPECT_EQ(-1, calculator.compute_commutative_order(2, 2));

  // 1 commute.
  EXPECT_EQ(-1, calculator.compute_commutative_order(2, 1));

  EXPECT_EQ(-1, calculator.compute_commutative_order(2, 6));

  // 1 commute.
  EXPECT_EQ(1, calculator.compute_commutative_order(2, 7));

  // 2 commutes.
  EXPECT_EQ(-1, calculator.compute_commutative_order(4, 7));
